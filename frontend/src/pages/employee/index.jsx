  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        ...
        {tasks.length === 0 ? <EmptyState message="No tasks assigned yet" icon={CheckSquare} />
          : (
      ...
      <div className="card">
        ...
        {timesheets.length === 0 ? <EmptyState message="No timesheets logged yet" icon={Clock} />
          : (
...
export function EmployeeTasksPage() {
  ...
  useEffect(() => {
    tasksApi.getMine().then(r => setTasks(r.data || [])).catch(() => toast.error('Failed')).finally(() => setLoading(false))
  }, [])
  ...
  return (
    <div>
      ...
      {tasks.length === 0
        ? <div className="card"><EmptyState message="No tasks assigned" icon={CheckSquare} /></div>
        : (
          <div className="space-y-3">
            {tasks.map(t => (
              <div key={t.id} className="card p-4 flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                  <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${t.priority === 'High' ? 'bg-red-500' : t.priority === 'Medium' ? 'bg-yellow-400' : 'bg-green-500'}`} />
