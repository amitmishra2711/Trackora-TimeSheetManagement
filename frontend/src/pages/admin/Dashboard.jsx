...
import { Users, UsersRound, FolderKanban, CheckSquare } from "lucide-react";
import {
  usersApi,
  teamsApi,
  projectsApi,
    ...
    const load = async () => {
      try {
        const [users, teams, projects, tasks, timesheets] = await Promise.all([
          ...
          timesheetsApi.getAll({ page: 1, pageSize: 10 }),
        ]);
        setStats({
        ...
        const allTs =
          (await timesheetsApi.getAll({ page: 1, pageSize: 100 })).data.items ||
          [];
        ...
        const byProject = {};
        allTs.forEach((ts) => {
          byProject[ts.projectName] =
            (byProject[ts.projectName] || 0) + Number(ts.hoursWorked);
        });
        setTimesheetData(
              ...
              {recentTimesheets.length === 0 ? (
                ...
                recentTimesheets.map((ts) => (
                  <tr key={ts.id}>
                    ...
                    <td>
                      <span className={`badge-${ts.status.toLowerCase()}`}>
                        {ts.status}
                      </span>
                    </td>
                  </tr>
