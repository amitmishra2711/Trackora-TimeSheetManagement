  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Team Members" value={team?.members?.length || 0} icon={Users} color="blue" />
        <StatCard label="Pending Timesheets" value={pending} icon={Clock} color="yellow" />
        <StatCard label="Approved" value={approved} icon={CheckSquare} color="green" />
        <StatCard label="Total Logged" value={timesheets.length} icon={FileText} color="indigo" />
      </div>

      {/* Team */}
      <div className="card p-5">
        <h3 className="font-semibold text-gray-900 mb-4">My Team — {team?.name || 'No team assigned'}</h3>
      ...
      <div className="card">
        ...
        <div className="table-wrap rounded-none">
          <table className="table">
            ...
            <tbody>
              {timesheets.filter(t => t.status === 'Pending').slice(0, 5).length === 0
                ? <tr><td colSpan={5} className="text-center text-gray-400 py-8">No pending timesheets</td></tr>
                : timesheets.filter(t => t.status === 'Pending').slice(0, 5).map(ts => (
                  <tr key={ts.id}>
...
export function LeaderTeamPage() {
  ...
  const load = async () => {
    setLoading(true)
    try { const res = await teamsApi.getLeading(); setTeam(res.data?.[0] || null) }
  ...
  const removeMember = async (userId) => {
    if (!team) return
    try { await teamsApi.removeMember(team.id, userId); toast.success('Member removed'); load() }
  ...
  return (
    <div>
      <PageHeader title={`My Team: ${team?.name || '—'}`} subtitle={`Leader: ${team?.leaderName || '—'}`} />
      <div className="card">
        {!team?.members?.length ? <EmptyState message="No team members" icon={Users} />
          : (
            <div className="divide-y divide-gray-100">
              {team.members.map(m => (
                <div key={m.id} className="flex items-center justify-between px-5 py-4">
                  ...
                  <button onClick={() => removeMember(m.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500">
                    <UserMinus size={16} />
                  </button>
                </div>
              ))}
...
export function LeaderReportPage() {
  ...
  const [form, setForm] = useState({ reportDate: new Date().toISOString().split('T')[0], summary: '', details: [] })
  ...
  return (
    <div>
      <PageHeader title="Daily Reports" subtitle="Submit end-of-day team reports"
        action={<button onClick={() => setModal(true)} className="btn-primary"><Plus size={16} /> Submit Report</button>} />

      {loading ? <div className="py-20 flex justify-center"><Spinner /></div>
        : reports.length === 0 ? <div className="card"><EmptyState message="No reports yet" icon={FileText} /></div>
          : (
              ...
              {reports.map(r => (
                <div key={r.id} className="card p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-medium text-gray-900">{new Date(r.reportDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Submit Daily Report" size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="date" className="input" value={form.reportDate} onChange={e => setForm(f => ({ ...f, reportDate: e.target.value }))} required />
          </div>
          <div>
            <label className="label">Summary</label>
            <textarea className="input" rows={4} placeholder="What did the team accomplish today?" value={form.summary} onChange={e => setForm(f => ({ ...f, summary: e.target.value }))} required />
          </div>
          <div>
              ...
              {tasks.map(t => (
                <div key={t.id}>
                  <label className="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer">
                    <input type="checkbox" checked={!!form.details.find(d => d.taskId === t.id)} onChange={() => toggleTask(t.id)} />
                    <span className="text-sm font-medium">{t.title}</span>
                    <span className="text-xs text-gray-400">{t.projectName}</span>
                  </label>
                  {form.details.find(d => d.taskId === t.id) && (
                    <input className="input mt-1 text-xs ml-6" placeholder="Notes..." value={form.details.find(d => d.taskId === t.id)?.notes || ''} onChange={e => setNote(t.id, e.target.value)} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <button type="submit" disabled={saving} className="btn-primary">{saving ? 'Submitting...' : 'Submit Report'}</button>
          </div>
        </form>
      </Modal>
    </div>
...
// Re-export shared pages for leader routes
