    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Welcome back, {user?.firstName}! You lead <span className="font-semibold text-indigo-600">{teams.length} team{teams.length !== 1 ? 's' : ''}</span>.
        </p>
      </div>
      ...
      <button onClick={() => navigate('/leader/team')}><StatCard label="Total Teams" value={teams.length} icon={Users} color="purple"  /></button>
        ...
        <button onClick={() => navigate('/leader/timesheets')}><StatCard label="Pending Approvals" value={pending} icon={Clock} color="yellow" /></button>
        <button onClick={() => navigate('/leader/timesheets')}><StatCard label="Approved" value={approved} icon={CheckSquare} color="green" /></button>
      </div>

      <div className="space-y-4">
        {teams.length === 0
          ? <div className="card p-6"><EmptyState message="No teams assigned" icon={Users} /></div>
          : teams.map(team => {
            const isExpanded = expandedTeam === team.id
            const teamTs = timesheets.filter(ts =>
              (team.members || []).some(m => m.id === ts.userId)
            )
            const teamPending = teamTs.filter(t => t.status === 'Pending').length
            ...
            return (
              <div key={team.id} className="card overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    {teamPending > 0 && (
                      <span className="badge-pending">{teamPending} pending</span>
                    )}
                    ...
                    {!team.members?.length
                      ...
                      : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {team.members.map(m => {
                            const memberTs = timesheets.filter(ts => ts.userId === m.id)
                            const memberPending = memberTs.filter(ts => ts.status === 'Pending').length
                            const memberHours = memberTs.reduce((s, ts) => s + Number(ts.hoursWorked), 0)
                            return (
                              <div key={m.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5">
                                    <span className="text-xs text-gray-400">{memberHours.toFixed(1)}h logged</span>
                                    {memberPending > 0 && (
                                      <span className="text-xs text-yellow-600 font-medium">{memberPending} pending</span>
                                    )}
                                  </div>
                                </div>
                              </div>
      ...
      <div className="card">
        ...
        <div className="overflow-x-auto">
          <table className="table">
            <thead><tr><th>Employee</th><th>Project</th><th>Date</th><th>Hours</th><th>Status</th></tr></thead>
            <tbody>
              {timesheets.filter(t => t.status === 'Pending').slice(0, 8).length === 0
                ? <tr><td colSpan={5} className="text-center text-gray-400 py-8">No pending timesheets</td></tr>
                : timesheets.filter(t => t.status === 'Pending').slice(0, 8).map(ts => (
                  <tr key={ts.id}>
...
export function LeaderTeamPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="My Teams"
        subtitle={`You lead ${teams.length} team${teams.length !== 1 ? 's' : ''}`}
      />
        ...
        : teams.map(team => (
          <div key={team.id} className="card overflow-hidden">
            ...
            <div className="px-5 py-4 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
              <div>
                <p className="font-semibold text-gray-900">{team.name}</p>
                <p className="text-xs text-gray-500 mt-0.5">{team.members?.length || 0} members</p>
              </div>
...
export function LeaderTasksPage() {
  ...
  const handleSave = async form => {
    ...
    try {
      if (modal?.id) {
        await import('../../api').then(m => m.tasksApi.update(modal.id, form))
      ...
      <div className="card">
        <div className="overflow-x-auto">
          <table className="table">
            <thead>
              <tr>
                <th>Title</th><th>Project</th><th>Assigned To</th>
                <th>Priority</th><th>Status</th><th>Due</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {tasks.length === 0
                ? <tr><td colSpan={7}><EmptyState message="No tasks yet" icon={CheckSquare} /></td></tr>
                : tasks.map(t => (
                  <tr key={t.id}>
                    ...
                    <td className="text-gray-500">{t.dueDate ? new Date(t.dueDate).toLocaleDateString() : '—'}</td>
                    <td>
                      <button
                        onClick={() => setModal(t)}
...
function LeaderTaskForm({ initial, projects, members, teams, onSave, onClose, loading }) {
  ...
  return (
    <form
      onSubmit={e => {
        e.preventDefault()
        onSave({ ...form, projectId: Number(form.projectId), assignedTo: Number(form.assignedTo) })
      }}
      className="space-y-4"
    >
      <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="label">Project <span className="text-xs text-gray-400">(your teams only)</span></label>
          <select className="input" value={form.projectId} onChange={set('projectId')} required disabled={isEdit}>
            <option value="">Select project...</option>
            {projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
        <div>
          <label className="label">Assign To <span className="text-xs text-gray-400">(your members only)</span></label>
          ...
          {form.projectId && filteredMembers.length === 0 && (
            <p className="text-xs text-red-500 mt-1">No members from your teams are on this project.</p>
          )}
        </div>
      </div>
...
export function LeaderReportPage() {
    ...
    const load = async () => {
      try {
        ...
        if (myTeams.length > 0) {
          ...
          const tsk = await import('../../api').then(m => m.tasksApi.getAll({ page: 1, pageSize: 100 }))
  ...
  const toggleTask = taskId => setForm(f => ({
    ...f,
  ...
  const setNote = (taskId, notes) =>
    setForm(f => ({ ...f, details: f.details.map(d => d.taskId === taskId ? { ...d, notes } : d) }))
  ...
  return (
    <div>
      ...
      {loading
        ...
        : reports.length === 0
          ? <div className="card"><EmptyState message="No reports yet for this team" icon={FileText} /></div>
          : (
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Submit Daily Report" size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="date" className="input" value={form.reportDate}
              onChange={e => setForm(f => ({ ...f, reportDate: e.target.value }))} required />
          </div>
          <div>
            <label className="label">Summary</label>
            <textarea className="input" rows={4} placeholder="What did the team accomplish today?"
              ...
              {tasks.map(t => (
                <div key={t.id}>
                  <label className="flex items-center gap-2 px-2 py-1 hover:bg-gray-50 rounded cursor-pointer">
                    <input type="checkbox" checked={!!form.details.find(d => d.taskId === t.id)}
                      onChange={() => toggleTask(t.id)} />
                    <span className="text-sm font-medium">{t.title}</span>
                    <span className="text-xs text-gray-400">{t.projectName}</span>
                  </label>
                  {form.details.find(d => d.taskId === t.id) && (
                    <input className="input mt-1 text-xs ml-6" placeholder="Notes..."
                      value={form.details.find(d => d.taskId === t.id)?.notes || ''}
