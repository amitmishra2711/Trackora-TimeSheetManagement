...
namespace Trackora.API.Services
{
    public class ProjectService : IProjectService
    {
        ...
        public async Task<PagedResult<ProjectDto>> GetAllAsync(PaginationQuery query)
        {
            ...
            if (!string.IsNullOrEmpty(query.Search))
                q = q.Where(p =>
                    ...
                    p.ProjectTeams.Any(pt => pt.Team.Name.Contains(query.Search)));
            var total = await q.CountAsync();
            var items = (await q.OrderByDescending(p => p.CreatedAt)
                .Skip((query.Page - 1) * query.PageSize).Take(query.PageSize).ToListAsync())
                .Select(Map).ToList();
            return new PagedResult<ProjectDto> { Items = items, TotalCount = total, Page = query.Page, PageSize = query.PageSize };
        }
        ...
        {
            var project = new Project { Name = dto.Name, Description = dto.Description, CreatedBy = adminId };
        ...
        public async Task<ProjectDto> UpdateAsync(int id, UpdateProjectDto dto)
        {
            ...
            p.Name = dto.Name; p.Description = dto.Description; p.Status = dto.Status;
            await _db.SaveChangesAsync();
        ...
        public async Task RemoveTeamAsync(int projectId, int teamId)
        {
            ...
            if (pt != null) { _db.ProjectTeams.Remove(pt); await _db.SaveChangesAsync(); }
        }

        public async Task<List<ProjectDto>> GetProjectsByTeamAsync(int teamId) =>
            (await _db.Projects.Include(p => p.Creator)
                .Include(p => p.ProjectTeams).ThenInclude(pt => pt.Team).ThenInclude(t => t.Leader)
                .Where(p => !p.IsDeleted && p.ProjectTeams.Any(pt => pt.TeamId == teamId)).ToListAsync())
            .Select(Map).ToList();
